...
export function AutomationPanel({ remote }: { remote: Snapshot | null }) {
  ...
  async function createRule(event: React.FormEvent) {
    ...
    setBusy(true); setError(''); setNotice(''); const rule: Rule = { id: 'local-' + crypto.randomUUID(), name: name.trim(), trigger: trigger.trim(), action: action.trim(), target, status: 'active' }
